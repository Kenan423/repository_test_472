...
```{r}
baltco_911_overdose_calls <- read_csv('tabula-Baltimore County; Carey, Samantha log OD.csv', col_names = FALSE)

baltco_911_overdose_calls <- baltco_911_overdose_calls |> rename(date = X1, time = X2, case_number = X3, event_type = X4, location = X5) |>
...
A1. July 14th and October 4th had the most number of calls at 23. After looking through the results, I do not believe there are any days with no overdose calls. The lowest number of calls per day I was able to find was 1.
...
A2. The days of the week are listed as numbers 1 through 7. The number of calls for each day of the week are listed. The percentage of calls that fall on each day of the week are listed in a column.
...
baltco_911_overdose_calls |> 
  ...
  summarise(calls = n()) |>
  mutate(pct_total = calls/4112*100) |>
...
A3. 4540 Silver Spring RD has the highest number of calls.
...
A4. The best story idea would be to do one on where and when overdoses are likely to occur.
