...
Then, using that `payee_name` column, write code to find all of the spending where Facebook is the payee and calculate the total amount of spending for each committee and present the results showing the committee that spent the most first. Which committee is that?

**A1.** Franchot Peter Friends Of.
...
read.csv('data/media_expenses_clean.csv')

media_expenses_clean |> group_by(payee_name) |> summarise(sum(amount))
...
**A2.** Peter Franchot and Jain Ashwani were both running for governor of Maryland. Billy Boniface was running for Harford County Executive. The ads for Peter Franchot generated at least a few thousand impressions, with some generating tens of thousands.
...
**A3.** Peter Franchot is the biggest recipient of media expenses by the Democratic primary winner.
...
**A4.** More money is spent on total online advertising than just Facebook.

```{r}

media_expenses_clean |> filter(str_detect(expense_purpose, 'Online Advertising')) |>
  group_by(committee_name) |>
  summarise(total_amount = sum(amount)) |>
arrange(desc(total_amount))
...
**A5** Cadidates spend a lot of money on Facebook ads in terms of total online advertising.
