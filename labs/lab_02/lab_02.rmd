...
**A1.** 26 departments have offered courses with "Climate" in the title and at least one seat. Atmospheric and Oceanic Science has offered the most classes.

```{r}
umd_courses |> filter(str_detect(title, "Climate") & seats> 0) |> 
group_by(department)|> 
summarize(classes = n()) |>
...
**A2.** Geology is the most interesting to me since it has a low number of classes with "Climate" in the title compared to other departments like Economics and Teaching and Learning.
...
**A3.** Business and Management has the most courses in the Fall 2023 term. 125 departments in this term have at least one syllabus posted. Business and Management, School of Music, and Information Studies are the only three departments that are in both top fives for departments.
...
**A4.** The spending has increased over time.
...
mutate(spending_location = case_when(
  str_detect(address, " Maryland ") ~ "In-state" ,
   str_detect(address, "  ") ~ "Out of state"
  ))
...
**A6.** How many of the expense methods were by 'Check'? 37,253

```{r}
maryland_expenses |>
filter(expense_method == 'Check')
