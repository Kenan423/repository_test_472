...
baltimore_911 <- read.csv('data/baltimore_911.csv') |>
  clean_names() |> mutate(zip_code = as.character(zip_code))

maryland_zcta <- read.csv('data/maryland_zcta.csv') |>
  clean_names() |> mutate(zcta5n = as.character(zcta5n))
...
A2. 314 GROVE NECK RD EARLEVILLE, MD in Cecil County had the most calls. That location is a drug and alcohol rehab center.
...
A3. 21217 has the highest rate of calls per 1,000 people. Some of the neighborhoods are Upton, Bolton Hill and Druid Heights. The population of this zip code is 9.4% White non-Hispanic and 81.5% of the population is under 18.
...
baltimore_911_with_demographics |>
  ...
  summarise(zip_code, population, pct_under_18, number= n()) |>
  mutate(per_capita = number/population*1000) |>
  arrange(desc(per_capita))

baltimore_911_with_demographics |>
  group_by(zip_code, neighborhood) |>
  filter(zip_code == 21217) |>
summarise(neighborhood, zip_code, pct_under_18, pct_white_non_hispanic)
...
A4: Which zip code has the highest percentage of homeowners?

```{r}

baltimore_911_with_demographics |>
 summarise(zip_code, pct_homeowner) |>
