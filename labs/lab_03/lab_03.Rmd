...
md_payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv", guess_max = 373564 ) |>
  clean_names()
...
A1:There are between 27 and 29 

```{r}
pg_schools <- md_payments |>
  filter(str_detect(vendor_name, "PRINCE GEORGE PUBLIC"),
...
```{r}
pg_schools |> group_by(fiscal_year) |> summarise(total_amount= sum())
...
md_payments |> filter(agency_name == "CAPITAL PROJECTS- UNIVERSITY OF MARYLAND") |>
