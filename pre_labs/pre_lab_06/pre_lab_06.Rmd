...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** The CSV files have the same column structure.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** The increase in people registered as "other" stands out to me.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows now. There should be 24 rows.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** It contains information on races for different counties as well as their population over the age of 18.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County has the highest percentage of black residents over the age of 18 at 60.9%. Prince George's County also has the highest percentage of democratic voters among black people over 18 in the state of Maryland at 77%.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard County has the highest Asian population over the age of 18 at 19%.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** The datatype of ZCTA5N is <chr>.
