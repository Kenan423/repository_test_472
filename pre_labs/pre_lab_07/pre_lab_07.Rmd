...
You may be asking: hey, wasn't there a 2020 Census? Where's that data? For the 2020 Census, not all data sets are available. But we can see some of them looking at the cd118 file from 2020. "cd118" stands for Congressional Distrcits 118, in reference to the [118th United States Congress](https://en.wikipedia.org/wiki/118th_United_States_Congress). That's because one of the first things that is done with Census data it's its Constitutional mandate: using the numbers of the Census to determine Congressional Districts.

**Task** Run the following codeblock and look at the resulting variable.

```{r}
cd118 <- load_variables(2020, "cd118", cache = TRUE)
```
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** It should include margin of error.
...
Is the difference between Calvert and Montgomery significant? **Answer** It is not significant.

Is the difference between Howard and everyone else significant? **Answer** It is significant.
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** The average income for schools with 60% or more is higher than the average income for schools with 40% or less. The number of households with less than 40% enrollment is greater than the number with over 60% enrollment.
...
**Task** What is another question you could ask of this data? **Answer** We could organize the percentages of college enrollment by zip code.
