...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** Some of the records are counted more than once because they are entered under different names. An example of this is "WASHINGTON CO COMMISSIONE" and "WASHINGTON CO COMMISSIONERS" being listed separately.
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** The refinr package capitalized certain data in the Grantee column that was not already capitalized.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** The column is only showing data that starts with W and they are listed in alphabetical order. All the names in "Grantee" and "grantee_clean" are capitalized.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** The changes made do not vary substantially from the the previous use of key_collision_merge.
